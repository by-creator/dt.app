    ...
    <style>
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .display-card {
            ...
            box-shadow: 0 16px 42px rgba(17, 34, 68, .08);
            border-radius: 24px;
            width: 100%;
            display: flex;
            flex-direction: row;
        ...
        .display-card.flash {
            box-shadow: 0 0 0 6px rgba(75,73,172,.25), 0 16px 42px rgba(17,34,68,.08);
        }
        ...
        .dc-left {
            ...
            justify-content: center;
            gap: 16px;
            background: linear-gradient(145deg, #f4f7ff 0%, #eef1ff 100%);
        ...
        .display-ticket-number {
            ...
            letter-spacing: -6px;
            line-height: 1;
            text-shadow: 0 6px 28px rgba(75,73,172,.22);
            transition: color .3s, transform .3s;
        }

        .display-ticket-number.highlight {
            color: var(--info);
        ...
        .display-guichet-block .gb-label {
            font-size: 12px;
            font-weight: 700;
            letter-spacing: 3px;
        ...
        .display-guichet-block .gb-value {
            font-size: 64px;
            font-weight: 900;
            color: #fff;
            line-height: 1;
        ...
        .display-guichet-block .gb-value.highlight { transform: scale(1.06); }
        ...
        .qr-card {
            ...
            border: 1px solid var(--border-soft);
            border-radius: 20px;
            box-shadow: 0 12px 32px rgba(17, 34, 68, .06);
        ...
        .qr-card:hover {
            box-shadow: 0 16px 40px rgba(75, 73, 172, .1);
        ...
        .qr-icon-wifi {
            background: linear-gradient(135deg, rgba(36,138,253,.2), rgba(121,120,233,.2));
            color: #248AFD;
        }

        .qr-icon-ticket {
            background: linear-gradient(135deg, rgba(121,120,233,.2), rgba(75,73,172,.2));
            color: #7978E9;
        }
        ...
        .qr-code img {
            ...
            padding: 5px;
            border: 1px solid var(--border-soft);
            box-shadow: 0 4px 10px rgba(17, 34, 68, .05);
        ...
        @media (max-width: 768px) {
            ...
            .dc-left .display-ticket-number { font-size: 100px; }
...
<body>
    <div class="activate-overlay" id="activate-overlay">
        ...
        <button class="btn-activate" onclick="activateAudio()">
            <i class="fas fa-play mr-2"></i> Demarrer l'affichage
        </button>
    </div>
    ...
    <div class="qr-section">
        <div class="qr-card">
            <div class="qr-card-left">
                <div class="qr-icon qr-icon-wifi">
            ...
            <div class="qr-card-right">
                <div class="qr-code">
                    <img src="https://api.qrserver.com/v1/create-qr-code/?size=320x320&data={{ urlencode($wifiQrData) }}" alt="QR Code WiFi">
                </div>
                <div class="qr-label">
                    <strong>SSID :</strong>
                    <span class="qr-text">{{ $wifiSsid }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="qr-card">
            <div class="qr-card-left">
                <div class="qr-icon qr-icon-ticket">
            ...
            <div class="qr-card-right">
                <div class="qr-code">
                    <img id="ticket-qr-img" src="" alt="QR Ticket" style="opacity:.4">
                </div>
    ...
    <script>
        ...
        function playChime() {
            ...
            tones.forEach((freq, i) => {
                ...
                osc.type = 'sine';
                const t = _audioCtx.currentTime + i * 0.25;
        ...
        function announce(numero, guichet) {
            if (!window.speechSynthesis) return;
            window.speechSynthesis.cancel();
            const utter = new SpeechSynthesisUtterance(`Ticket ${numero}, veuillez vous presenter au ${guichet}`);
            utter.lang = 'fr-FR';
            utter.rate = 0.88;
        ...
        function updateDisplay(ticket) {
            ...
            document.getElementById('agent-name').textContent = ticket.agentNom || '';

            const card = document.getElementById('display-card');
            const numEl = document.getElementById('ticket-number');
            const guichetEl = document.getElementById('guichet-name');
            card.classList.add('flash');
            numEl.classList.add('highlight');
            guichetEl.classList.add('highlight');
            setTimeout(() => {
                card.classList.remove('flash');
                numEl.classList.remove('highlight');
                guichetEl.classList.remove('highlight');
            }, 2000);
        }
        ...
        async function refreshTicketQr() {
            try {
                ...
                const qrUrl = 'https://api.qrserver.com/v1/create-qr-code/?size=320x320&data=' + encodeURIComponent(ticketUrl);
            ...
            _qrRefreshTimer = setTimeout(refreshTicketQr, 4 * 60 * 1000);
        }
        ...
        function connectWS() {
            ...
            client.connect({}, function () {
                ...
                client.subscribe('/topic/tickets/called', function (msg) {
                    const ticket = JSON.parse(msg.body);
                    updateDisplay(ticket);
                    playChime();
                    const guichet = ticket.guichetNumero || 'guichet';
                    setTimeout(() => announce(ticket.numero, guichet), 950);
                });

                client.subscribe('/topic/tickets/closed', function () {
                    setTimeout(resetDisplay, 3000);
                });

                client.subscribe('/topic/tickets/created', function () {
                    refreshTicketQr();
                });
            }, function () {
        ...
        document.addEventListener('DOMContentLoaded', refreshTicketQr);
    </script>
</body>
</html>
