<x-layouts::app :title="__('Gfa Admin')">
    <div class="gfa-admin-page flex h-full w-full flex-1 flex-col gap-6 pb-8">
        ...
        <script>
            ...
            async function updateService(id) { try { await fetch(`/gfa/api/services/${id}`, { method:'PUT', headers:csrfHeaders({ 'Content-Type':'application/json' }), body:JSON.stringify({ nom:$(`service-name-${id}`).value.trim(), prefixe:$(`service-prefix-${id}`).value.trim() }) }); showStatus('service-status', 'Service modifie avec succes.', true); await loadServices(); } catch (e) { showStatus('service-status', 'Erreur lors de la modification.', false); } }
            async function deleteService(id) { const result = await Swal.fire({ title:'Supprimer ce service ?', icon:'warning', showCancelButton:true, confirmButtonText:'Supprimer', cancelButtonText:'Annuler', confirmButtonColor:'#dc3545' }); if (!result.isConfirmed) return; try { await fetch(`/gfa/api/services/${id}`, { method:'DELETE', headers:csrfHeaders() }); showStatus('service-status', 'Service supprime avec succes.', true); await loadServices(); } catch (e) { showStatus('service-status', 'Erreur lors de la suppression.', false); } }
            async function updateGuichet(id) { try { await fetch(`/gfa/api/guichets/${id}`, { method:'PUT', headers:csrfHeaders({ 'Content-Type':'application/json' }), body:JSON.stringify({ numero:$(`guichet-numero-${id}`).value.trim(), infos:$(`guichet-infos-${id}`).value.trim(), serviceId:$(`guichet-service-${id}`).value || null }) }); showStatus('guichet-status', 'Guichet modifie avec succes.', true); await loadGuichets(); } catch (e) { showStatus('guichet-status', 'Erreur lors de la modification.', false); } }
            async function deleteGuichet(id) { const result = await Swal.fire({ title:'Supprimer ce guichet ?', icon:'warning', showCancelButton:true, confirmButtonText:'Supprimer', cancelButtonText:'Annuler', confirmButtonColor:'#dc3545' }); if (!result.isConfirmed) return; try { await fetch(`/gfa/api/guichets/${id}`, { method:'DELETE', headers:csrfHeaders() }); showStatus('guichet-status', 'Guichet supprime avec succes.', true); await loadGuichets(); await loadAgents(); } catch (e) { showStatus('guichet-status', 'Erreur lors de la suppression.', false); } }
            async function updateAgent(id) { try { await fetch(`/gfa/api/agents/${id}`, { method:'PUT', headers:csrfHeaders({ 'Content-Type':'application/json' }), body:JSON.stringify({ nom:$(`agent-nom-${id}`).value.trim(), prenom:$(`agent-prenom-${id}`).value.trim(), serviceId:$(`agent-service-${id}`).value || null, guichetId:$(`agent-guichet-${id}`).value || null }) }); showStatus('agent-status', 'Agent modifie avec succes.', true); await loadAgents(); } catch (e) { showStatus('agent-status', 'Erreur lors de la modification.', false); } }
            async function deleteAgent(id) { const result = await Swal.fire({ title:'Supprimer cet agent ?', icon:'warning', showCancelButton:true, confirmButtonText:'Supprimer', cancelButtonText:'Annuler', confirmButtonColor:'#dc3545' }); if (!result.isConfirmed) return; try { await fetch(`/gfa/api/agents/${id}`, { method:'DELETE', headers:csrfHeaders() }); showStatus('agent-status', 'Agent supprime avec succes.', true); await loadAgents(); } catch (e) { showStatus('agent-status', 'Erreur lors de la suppression.', false); } }
