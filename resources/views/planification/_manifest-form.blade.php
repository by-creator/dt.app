...
<div class="pm-card">
    ...
    <form class="pm-form" method="POST" action="{{ route('planification.upload-manifest.store') }}" enctype="multipart/form-data">
        @csrf
        <div>
            <label class="pm-label" for="manifest-file">Fichier manifeste</label>
            <input id="manifest-file" type="file" name="manifest" class="pm-file" accept=".pdf,.xlsx,.xls,.csv" required>
        </div>
        <button type="submit" class="pm-btn">
            <i class="fas fa-upload"></i> Importer le manifeste
        </button>
    </form>
</div>
